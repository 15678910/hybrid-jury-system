...
**❌ 이런 기준은 틀렸다** (2026-08-17 에 실제로 잘못 안내했다):
- 「주소창이 보이면 브라우저(클라우드), 안 보이면 앱(로컬)」 — **틀렸다.** 데스크탑 앱
  안에서도 클라우드 세션이 열린다. 주소창은 아무것도 알려 주지 않는다.
- 「데스크탑 앱이면 로컬이다」 — **틀렸다.** 같은 이유다.

**⭕ 화면에서 클라우드 세션을 알아보는 표시** (사용자 스크린샷으로 확인):
- **「세션 재개됨」** 표시 — 예전 대화를 이어 연 것. 클라우드 세션만 이어 열린다
- 입력창 위의 **브랜치 칩**(`claude/…`)과 **변경량 표시**(`+305,489 −13`)
- 그 옆의 **「PR 생성」 버튼**

로컬 폴더로 연 세션에는 이 줄이 없다. **브랜치 칩과 PR 버튼이 보이면 클라우드다.**

**⭕ 가장 확실한 판별 — AI 가 직접 확인한다**: `ls functions/.env`.
있으면 로컬, 없으면 클라우드. 화면 표시를 읽을 필요 없이 한 줄로 끝난다.
