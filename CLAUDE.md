...
- **사건**: 민주당 검찰개혁안 업데이트 시, 사용자의 제안(우리 안/핀란드식 모델)의 내용을 민주당 공식 입장으로 잘못 기재
- **오염된 데이터**: 시민기소심사위원회(강제기소권), 참심제(시민3+법관1), 법률감찰관+사법옴부즈만 → 이것은 전부 **우리 안**이지 민주당 안이 아님
- **원인**: 사용자가 제공한 이미지의 두 번째 표(헌법 제1조 제2항 구현)를 민주당 입장으로 착각. 실제로는 사용자의 제안
- **교훈**:
  1. **출처 확인 필수**: 정당/기관 입장에는 해당 정당이 실제 발표한 내용만 기재
  2. **우리 안 vs 타 정당 안 구분**: 사용자 제안(핀란드 모델)을 다른 정당 입장으로 절대 혼동 금지
  3. **원칙 vs 구체안 구분**: "국민 통제 필요" 원칙만 언급한 경우, 구체적 제도(시민기소심사위 등)를 해당 정당이 주장한 것으로 기재 금지
  4. **확인 불가 시 질문**: 출처가 불분명한 내용은 사용자에게 확인 후 기재
  5. **비교 시 주체 명확 표시**: "민주당 안 일치"가 아닌 "우리 안" vs "민주당 안" 명확 구분
...
- **평소에는 데스크탑에서 Claude Code 를 켠다.** 폴더를 직접 보므로 파일 왕복이 없고,
  함수 배포·로컬 프리뷰·판례 수집이 모두 된다.
- **외출 중에도 데스크탑을 쓴다.** 크롬 원격 데스크톱(`remotedesktop.google.com`)으로
  데스크탑 화면에 들어가 터미널에서 Claude Code 를 켜면 제약이 전혀 없다. 데스크탑을
  켜 둔 채(절전 해제) 나가는 것이 조건이다. 이 경우 `public/` 에 넣은 파일은 커밋하지
  않아도 바로 보인다 — 그 화면이 데스크탑 자체이기 때문이다.
...
### ⚠️ 가장 헷갈리는 함정 — 화면이 아니라 「어떻게 켰는가」가 가른다

**데스크탑 화면 앞에 있어도 원격 세션일 수 있다.** 갈리는 것은 앉은 자리가 아니라
Claude Code 를 켠 방법이다.

| 여는 방법 | 실제로 도는 곳 | 폴더가 보이나 |
|---|---|---|
| 브라우저에서 `claude.ai/code` → **사이드바에서 세션 클릭** | **클라우드 VM** | ❌ GitHub 복제본만 |
| **터미널에서 `claude` 실행** | 그 컴퓨터 | ✅ 실제 폴더 |

사이드바의 세션 목록은 클라우드에 저장된 세션들이다. 데스크탑에서 켠 Claude Code 는
거기에 나타나지 않는다. 그러므로 **데스크탑(또는 크롬 원격 데스크톱) 화면에서
브라우저로 예전 세션을 이어 여는 것은 데스크탑 세션이 아니다.**

2026-08-08 에 이것 때문에 하루를 소모했다 — 사용자는 데스크탑 앞에 있었고 `public/` 에
이미지를 넣었는데, AI 는 클라우드에서 돌고 있어 그 파일을 볼 수 없었다.

데스크탑 세션으로 열려면: 탐색기에서 프로젝트 폴더 → 주소창에 `powershell` → 엔터 → `claude`.
(대신 그 세션은 이전 대화를 모르므로 `docs/SESSION_HANDOFF_*.md` 를 읽혀 이어붙인다.)
...
```bash
ls functions/.env    # 있으면 데스크탑, 없으면 원격
```
