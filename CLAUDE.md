...
|---|---|---|
| **Claude 데스크탑 앱** (시작 메뉴에서 실행) | 그 컴퓨터 | ✅ 실제 폴더 |
| **터미널에서 `claude` 실행** | 그 컴퓨터 | ✅ 실제 폴더 |
| 브라우저에서 `claude.ai/code` → **사이드바에서 세션 클릭** | **클라우드 VM** | ❌ GitHub 복제본만 |

**앞의 둘은 차이가 없다.** 데스크탑 앱과 터미널은 생김새만 다를 뿐(창이냐 검은 화면이냐)
둘 다 그 컴퓨터에서 돌고, 로컬 폴더도 함수 배포도 똑같이 된다. **평소 쓰는 것은 데스크탑
앱이다.** 터미널은 「확실히 로컬인지」 의심스러울 때의 확인용 정도로 생각하면 된다.

**다른 것은 세 번째뿐이다.** 브라우저 창 안에서 열리면 클라우드다. 구분법은 간단하다 —
**주소창이 보이면 브라우저(클라우드), 안 보이면 앱(로컬).**

브라우저로 가게 되는 길은 여럿이라 자기도 모르게 걸린다: 주소창에 `claude.ai` 입력,
북마크, **이메일·알림의 링크 클릭**, 검색 결과 클릭. 데스크탑 앱이 켜져 있는데도
알림 링크를 눌러 브라우저로 들어가면 그 순간부터 클라우드다.

사이드바의 세션 목록은 클라우드에 저장된 세션들이다. 데스크탑에서 켠 Claude Code 는
거기에 나타나지 않는다. 그러므로 **데스크탑(또는 크롬 원격 데스크톱) 화면에서
브라우저로 예전 세션을 이어 여는 것은 데스크탑 세션이 아니다.** 데스크탑 세션에는
...
2026-08-17 에 같은 일이 반복됐다. 원인은 8월 2일에 브라우저로 시작한 세션을 보름 동안
사이드바에서 이어 열고 있었던 것이다. **세션이 도는 곳은 만들어질 때 정해지고 바뀌지 않는다.**
원격 세션을 데스크탑 세션으로 전환하는 방법은 없다 — 새로 시작하는 수밖에 없다.

**데스크탑 앱이 안 열릴 때**: 트레이(시계 옆 `∧`)로 내려가 있는 경우가 많다.
가장 확실한 방법은 시작 메뉴에서 `Claude` 검색 → 클릭. 앱을 연 뒤 프로젝트 폴더를 선택해야 한다.
터미널로 열려면: 탐색기에서 프로젝트 폴더 → 주소창에 `powershell` → 엔터 → `claude`.
