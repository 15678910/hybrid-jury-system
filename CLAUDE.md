...
## 🚨 피고인 데이터 필수 규칙 (2026-02-23 제정)

### 필수 4개 필드 — 모든 피고인에게 반드시 존재해야 함
무죄 선고를 받은 피고인이라도 혐의가 있으면 4개 필드 모두 필요합니다.

| 필드 | 설명 | 무죄인 경우 |
|------|------|------------|
| `sentencingGuidelines` | 양형기준 비교 (권고형, 가중/감경 요소) | "유죄 시" 기준으로 작성 |
| `judgeHistory` | 판사 판결 이력 (재판장 정보, 경력) | 동일하게 필요 |
| `keyIssues` | 핵심 쟁점 (검찰/변호인/재판부 판단) | 무죄 판단 근거 쟁점 작성 |
| `sources` | 자료 출처 (언론사, URL, 날짜) | 동일하게 필요 |

### 피고인 추가/수정 시 체크리스트
1. 기본 정보: `name`, `title`, `status`, `keyFacts`, `charges` 확인
2. **sentencingGuidelines**: 각 혐의별 양형기준 권고형 + 가중/감경 요소 + 재판부 분석
3. **judgeHistory**: 재판장 이름, 직위, 소속, 경력사항, 특이사항
4. **keyIssues**: 최소 1개 이상의 쟁점 (검찰 주장 / 변호인 주장 / 재판부 판단)
5. **sources**: 최소 2개 이상 언론 출처 (`{ label, title, url, date }`)
6. 수정 완료 후 반드시 해당 피고인 상세 페이지에서 모든 탭 확인

### 현재 데이터 완전성 현황 (2026-02-23 기준)
- ✅ 4개 필드 완비: 김봉식, 김용군, 목현태, 조지호 (4명)
- ⚠️ sources만 누락: 김건희, 김용현, 김주현, 노상원, 윤석열, 윤승영, 이상민, 이완규, 한덕수 (9명)
- ❌ 4개 필드 모두 미보유: 곽종근, 김태효, 문상호, 박성재, 박안수, 박종준, 심우정, 여인형, 이진우, 조태용, 최상목, 추경호 (12명)

### 위반 방지
- **새 피고인 데이터 추가 시**: 위 4개 필드가 모두 포함되어야 완료로 간주
- **기존 피고인 수정 시**: 누락 필드가 있으면 함께 보강
- **1심 선고 결과 반영 시**: 반드시 sources 출처도 함께 추가
...
- [ ] 모바일 반응형 확인
- [ ] 관리자 페이지 접근 가능
- [ ] 양형분석 피고인 데이터: 신규/수정 대상의 필수 4개 필드 확인
