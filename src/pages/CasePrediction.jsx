    ...
    return (
        <div className="bg-amber-50 border-l-4 border-amber-500 p-5 rounded-r-lg mb-8">
            <div className="flex items-start gap-3">
                ...
                <div>
                    <h3 className="font-bold text-amber-900 mb-1">아직 확률을 표시하지 않습니다</h3>
...
function CaseCard({ c }) {
    ...
    return (
        <div className="bg-white rounded-xl border shadow-sm p-6 mb-6">
            ...
            <p className="text-gray-600 mb-5 leading-relaxed">{c.summary}</p>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b-2 border-gray-200">
                                <th className="text-left py-2 px-3 font-semibold text-gray-700">예상 결론</th>
                                <th className="text-right py-2 px-3 font-semibold text-gray-700">확률 구간</th>
                            </tr>
                        </thead>
                        <tbody>
                            {c.scenarios.map((s) => (
                                <tr key={s.label} className="border-b border-gray-100">
                                    <td className="py-2.5 px-3 text-gray-800">{s.label}</td>
                                    <td className="py-2.5 px-3 text-right font-bold text-gray-900">{s.range}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            {c.unverified?.length > 0 && (
                <div className="mt-6">
                    <h4 className="font-bold text-gray-800 mb-3 text-sm">확인해야 할 사실</h4>
                    <div className="space-y-3">
                        {c.unverified.map((u, i) => (
                            <div key={i} className="border-l-4 border-amber-400 bg-amber-50/60 pl-4 py-3 rounded-r">
                                <p className="font-medium text-gray-900 text-sm mb-1">⚠️ {u.claim}</p>
                                <p className="text-gray-700 text-sm leading-relaxed mb-1">{u.why}</p>
                                <p className="text-gray-500 text-xs">확인 방법: {u.howToVerify}</p>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    return (
        <div className="space-y-8">
            <section className="bg-white rounded-xl border shadow-sm p-6">
                <h3 className="text-lg font-bold text-gray-900 mb-4">확률 하나가 아니라 숫자 두 개에서 나옵니다</h3>
            ...
            <section className="bg-white rounded-xl border shadow-sm p-6">
                <h3 className="text-lg font-bold text-gray-900 mb-4">기저율 — 예측의 출발점</h3>
                <p className="text-gray-700 leading-relaxed mb-4">
                    ...
                    <table className="w-full text-sm">
                        <tbody>
                            <tr className="border-b border-gray-100">
                                <td className="py-2.5 pr-4 text-gray-600 whitespace-nowrap">전합 형사사건 파기율</td>
                            ...
                            <tr className="border-b border-gray-100">
                                <td className="py-2.5 pr-4 text-gray-600">표본 수</td>
                                <td className="py-2.5 text-gray-900">{br.sampleSize ?? '—'}</td>
                            </tr>
                            <tr>
                                <td className="py-2.5 pr-4 text-gray-600 align-top">출처</td>
                                <td className="py-2.5 text-gray-900">{br.source}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <p className="text-gray-500 text-xs mt-4 leading-relaxed border-t pt-4">
                    <strong>표본의 성격:</strong> {br.note}
                </p>
            </section>

            <section className="bg-white rounded-xl border shadow-sm p-6">
                <h3 className="text-lg font-bold text-gray-900 mb-2">전원합의체 회부가 뜻하는 것</h3>
                ...
                <blockquote className="border-l-4 border-blue-400 bg-blue-50/60 pl-4 py-3 rounded-r mb-4">
                    <p className="text-sm font-semibold text-gray-900 mb-2">{EN_BANC_GROUNDS.article}</p>
                    <p className="text-sm text-gray-700 leading-relaxed">{EN_BANC_GROUNDS.principle}</p>
                    <ol className="list-decimal list-inside text-sm text-gray-700 mt-3 space-y-1">
                        {EN_BANC_GROUNDS.grounds.map((g, i) => <li key={i}>{g}</li>)}
                    </ol>
                </blockquote>
                <p className="text-gray-800 leading-relaxed">{EN_BANC_GROUNDS.implication}</p>
                <a
            ...
            <section className="bg-gray-900 text-white rounded-xl p-6">
                <h3 className="text-lg font-bold mb-4">이 페이지가 지키는 것</h3>
    ...
    if (!SCORECARD.length) {
        return (
            <div className="bg-white rounded-xl border shadow-sm p-8 text-center">
                <p className="text-gray-800 font-medium mb-2">아직 검증할 예측이 없습니다.</p>
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white rounded-xl border shadow-sm p-6">
                <p className="text-gray-600 text-sm mb-1">전체 적중률</p>
            ...
            {SCORECARD.map((s) => (
                <div key={s.caseId} className="bg-white rounded-xl border shadow-sm p-5">
                    <div className="flex items-center gap-2 mb-2">
                        <span className={`text-xs px-2 py-1 rounded-full font-medium ${s.hit ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            {s.hit ? '적중' : '빗나감'}
                        </span>
                        <h4 className="font-bold text-gray-900">{s.title}</h4>
                    </div>
                    <p className="text-sm text-gray-600">예측: {s.predicted} · 실제: {s.actual}</p>
                    {s.note && <p className="text-sm text-gray-500 mt-2">{s.note}</p>}
                </div>
            ))}
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="max-w-4xl mx-auto px-4 py-10">
                <div className="mb-8">
                    <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">재판 결과 예측</h1>
                    <p className="text-gray-600 leading-relaxed">
                        확률을 팔지 않고 근거를 팝니다. 숫자보다 <strong>그 숫자가 어디서 나왔는지</strong>를 먼저 보여주고,
                        예측한 뒤에는 <strong>맞았는지 틀렸는지를 남깁니다.</strong>
                    </p>
                </div>
                ...
                <div className="flex gap-2 mb-8 border-b overflow-x-auto">
                    {TABS.map((t) => (
                        <button
                            key={t.id}
                            onClick={() => setTab(t.id)}
                            className={`px-4 py-2.5 font-medium whitespace-nowrap transition border-b-2 -mb-px ${
                                tab === t.id
