...
const CRIMINAL_PROCEDURE_BILLS = {
        ...
        articles: [
            ...
            },
            {
                ...
                remedy: [
                    ...
                    '★ 다만 국선 확대만으로는 부족하다 — 국선변호인의 선정·관리감독 주체가 법원이고 국선전담변호사는 2년마다 재위촉된다. 일감을 주는 쪽과 재판하는 쪽이 같으면 재판부와 각을 세우는 변론에 구조적 억제가 걸린다. 선정·감독권을 법원에서 떼어 독립 기구(일본 사법지원센터 모델)나 변호사단체로 이관하고, 사건당 55만 원에 머무는 보수와 2015년 이후 동결된 국선전담 보수를 현실화해야 한다. 조력자가 독립적이지 않으면 늘어난 국선은 늘어난 방어가 되지 않는다.',
    ...
    roadmap: {
        ...
        accessCost: {
            ...
            malpractice: {
                title: '조력의 부실을 누가 입증하는가 — 의뢰인의 2중 부담',
                body: '조력을 살 수 없는 사람이 권리에서 배제된다면, 조력을 산 사람에게는 또 다른 부담이 남는다. 선임한 변호인이 공판기일에 나오지 않거나 서면을 내지 않았을 때, 보수를 돌려받거나 징계를 구하려면 그 사실을 의뢰인이 증명해야 한다. 재판 결과를 감당하는 것도, 조력이 부실했다는 증거를 모으는 것도 같은 사람이다.',
                key: '★ 그런데 그 증거는 이미 국가가 갖고 있다. 공판조서에는 기일마다 출석자가 기재되고 피고인은 그 열람·등사를 청구할 수 있으며, 법원이 응하지 않으면 그 조서를 유죄의 증거로 쓸 수 없다(형사소송법 제55조①③). 더구나 필요적 변호사건에서는 변호인 없이 개정하지 못하고, 변호인이 출석하지 않으면 법원이 직권으로 새 국선변호인을 선정해야 한다(제282조·제283조). 즉 변호인이 나오지 않은 순간 재판부는 이미 알고 있고 조치까지 취한다. 다만 그 사실이 의뢰인에게 자동으로 가지 않을 뿐이다. 그래서 해법은 「입증」이 아니라 「통지」에 있다.',
                fixes: [
                    { n: '1', name: '불출석 사실의 의뢰인 통지 (신설 필요)', desc: '선임 변호인이 기일에 출석하지 않으면 법원이 그 사실을 피고인 본인에게 통지하도록 한다. 법원이 이미 알고 있는 사실을 한 줄 알리는 것이라 비용은 사실상 0인데, 통지서 자체가 증거가 되므로 의뢰인의 입증 부담이 통째로 사라진다.' },
                    { n: '2', name: '법원 → 변호사회 통보 (신설 필요)', desc: '무단 불출석을 재판부가 소속 지방변호사회에 통보하도록 한다. 현재 징계 절차의 출발점은 언제나 피해자의 신고인데, 사실을 먼저 아는 쪽은 법원이다.' },
                    { n: '3', name: '입증책임의 조정', desc: '기일 불출석·서면 미제출처럼 기록으로 확인되는 객관적 불이행에 대해서는 정당한 사유를 변호사가 밝히도록 한다. 변론의 질(質)까지 전환하자는 것이 아니라, 다투는 대상을 「했는가 안 했는가」로 한정하는 것이다.' },
                ],
                existing: '※ 이미 있는 통로 — 지방변호사회는 변호사와 위임인 사이의 직무상 분쟁을 당사자 청구로 조정할 수 있고(변호사법 제74조), 의뢰인·법정대리인·배우자·직계친족·형제자매는 소속 지방변호사회의 장에게 징계개시를 청원할 수 있다(제97조의3). 회장은 지체 없이 신청 여부를 결정해 결과와 이유의 요지를 통지해야 하고, 기각되거나 접수 후 3개월이 지나도 결정이 없으면 14일 이내에 대한변협 회장에게 재청원할 수 있다. 변협징계위원회는 징계개시 청구를 받은 날부터 6개월 이내에 결정해야 한다(제98조).',
                gap: '문제는 실효성이다. 대한변협이 최근 10년간 「성실의무 위반」으로 내린 징계는 138건이고 그중 견책·과태료 등 경징계가 80%였다(2023년 국정감사). 공개도 마찬가지다 — 변호사법 제98조의5는 징계처분을 협회 홈페이지에 3개월 이상 게재하고 「변호사를 선임하려는 자」가 신청하면 열람·등사를 제공하도록 하는데, 최근 3년간 열람 신청은 10건에 그쳤다. 참여연대가 별도로 「변호사징계정보찾기」를 운영하는 것 자체가 공적 공개가 시민에게 닿지 않는다는 증거다. 앞의 가격 공시와 정확히 같은 구조 — 데이터는 이미 있고 출구만 없다. 그리고 국선의 경우 이것은 삼중 종속의 뒷면이다 — 불성실한 국선변론을 가장 먼저 아는 사람은 재판부인데, 그 재판부가 바로 국선변호인을 선정하고 보수를 정하는 주체다. 감독자와 발주자가 같으면 감독은 발주의 편의에 종속된다.',
            },
            steps: {
            ...
            conclusion: '정리하면 가장 빨리 손댈 수 있는 것은 세 가지다 — 변호사법 제28조의2에 공표 조항 한 항을 추가하는 것(데이터는 이미 있다), 법무사 업무 범위를 유권해석으로 확인하는 것(법 개정이 필요 없을 수 있다), 그리고 변호인 불출석 사실을 법원이 의뢰인에게 통지하도록 하는 것(법원이 이미 아는 사실이라 비용이 0에 가깝다). 그리고 가장 근본적인 것은 공설변호인 사무소와 서식의 자기완결화다. 국선을 늘리는 것은 「법률가를 사주는」 방식이고, 서식을 단순화하고 시민이 직접 판단하는 통로를 만드는 것은 「법률가가 필요 없게 하는」 방식이다. 비용 장벽을 넘게 해주는 것과, 비용 장벽이 필요 없는 통로를 만드는 것은 다른 일이다.',
        },
        institutional: {
                                        ...
                                        <div className="bg-white rounded-xl shadow-sm border overflow-hidden mb-4">
                                            ...
                                            <div className="p-4 space-y-3">
                                                ...
                                                <div className="bg-rose-50 border border-rose-200 rounded-lg p-3">
                                                    <p className="text-base font-bold text-rose-900 mb-2">⚖️ {CRIMINAL_PROCEDURE_BILLS.roadmap.accessCost.malpractice.title}</p>
                                                    <p className="text-base text-rose-900 leading-relaxed mb-2">{CRIMINAL_PROCEDURE_BILLS.roadmap.accessCost.malpractice.body}</p>
                                                    <div className="bg-white border-l-4 border-rose-500 px-3 py-2 rounded-r mb-2">
                                                        <p className="text-base font-bold text-rose-900 leading-relaxed">{CRIMINAL_PROCEDURE_BILLS.roadmap.accessCost.malpractice.key}</p>
                                                    </div>
                                                    <div className="space-y-2 mb-2">
                                                        {CRIMINAL_PROCEDURE_BILLS.roadmap.accessCost.malpractice.fixes.map((f, fi) => (
                                                            <div key={fi} className="bg-white rounded px-3 py-2 border border-rose-100">
                                                                <p className="text-base font-bold text-rose-900 mb-0.5">{f.n}. {f.name}</p>
                                                                <p className="text-base text-gray-700 leading-relaxed">{f.desc}</p>
                                                            </div>
                                                        ))}
                                                    </div>
                                                    <p className="text-base text-rose-900 leading-relaxed mb-2">{CRIMINAL_PROCEDURE_BILLS.roadmap.accessCost.malpractice.existing}</p>
                                                    <p className="text-base text-gray-700 leading-relaxed">{CRIMINAL_PROCEDURE_BILLS.roadmap.accessCost.malpractice.gap}</p>
                                                </div>
