...
export default function Videos() {
    ...
    const shareToTwitter = (video, videoId) => {
        ...
        window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(tweetText)}&url=${encodeURIComponent(videoUrl)}`, '_blank', 'width=600,height=400');
    };
                                        ...
                                        {cardShorts.map((sv) => (
                                            <div key={sv.key} className={`bg-white rounded-xl shadow-md hover:shadow-lg transition-all overflow-hidden ${sv.wide ? 'col-span-2' : ''}`}>
                                                <div className="relative bg-black">
                                                    ...
                                                    {(sv.kind === '제안편' || sv.kind.startsWith('해설편')) && (
