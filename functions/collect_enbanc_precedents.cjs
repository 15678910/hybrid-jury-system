...
(async () => {
    ...
    if (withCompare && panelRows.length) {
        ...
        // 소부는 표본이 훨씬 크다. 방향을 볼 수 있는 것은 사실상 이쪽이므로 함께 찍는다.
        // 앞서 이 줄이 없어 전합 5건만 보고 방향을 논할 뻔했다.
        console.log(`  ▶ 방향 — 피고인 ${sp.피고인쪽파기}건(${pct(sp.피고인쪽비율)}) / `
            + `검사 ${sp.검사쪽파기}건(${pct(sp.검사쪽비율)}) / `
            + `판정불가 ${sp.방향불명}건 (판정률 ${pct(sp.방향판정률)})`);
        if (s.파기율 && sp.파기율) {
